...
- R3-UX-P01 授權 baseline：`aa4f2d508df484b8bba58416c77a3d1760daac75`。
- 目前 working branch：`feat/r3-ux-p01-locale-reduction`；maintenance patch 尚未建立 commit。
...
**R3-UX-P01 Locale Scope Reduction：IMPLEMENTED / READY FOR HUMAN REVIEW。**

R3-SUX-01 與 R3-SUX-02 均已關閉並整合至 `main`。R3-UX-P01 已依產品決策將 supported locales 縮減為 `zh-CN`／`en-US`，移除 `zh-TW` visible option 與正式繁中字典，並將 URL、localStorage、browser／WebView 的 legacy `zh-TW`／`zh-Hant` 輸入 normalize 至 `zh-CN`；final fallback 為 `zh-CN`。此狀態等待 Human UX Review，不代表 patch gate PASS。R3-SUX-03 為 `NOT AUTHORIZED`，Production Implementation 維持 `BLOCKED`。
...
- `zh-CN`、`en-US` 的 locale resolution 與翻譯；legacy `zh-TW`／`zh-Hant` 輸入 normalize 至 `zh-CN`。
...
- Global shell 已提供 `zh-CN`、`en-US` visible locale switcher；`zh-TW` 已移除，legacy `zh-TW`／`zh-Hant` 輸入 normalize 至 `zh-CN`，final fallback 為 `zh-CN`。切換時保留既有 query state，並沿用 browser-only `portal_lang` preference。
...
目前下一個獲授權的動作僅為 R3-UX-P01 Human UX Review。R3-SUX-03 為 `NOT AUTHORIZED`，Production Implementation 為 `BLOCKED`；未取得明確授權前，不得 commit、push、merge、開始下一 Stage 或加入任何 Production integration。
