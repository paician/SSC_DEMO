...
const { createTranslator, resolveLocale, localeUrl, LOCALE_STORAGE_KEY, SUPPORTED_LOCALES } = await import('../data/i18n.js');
...
// 17-18. Prototype identification and all new supported-locale keys.
const newLocaleKeys = [
...
const dictionaryEntries = [...i18nSource.matchAll(/const\s+(ZH_CN|EN)\s*=\s*Object\.freeze\(\{([\s\S]*?)\n\}\);/g)]
  .map(match => ({ name: match[1], keys: [...match[2].matchAll(/^\s+"([^"]+)":/gm)].map(entry => entry[1]) }));
assert.deepEqual(dictionaryEntries.map(item => item.name), ['ZH_CN', 'EN']);
assert.ok(dictionaryEntries[0].keys.length > 300, 'Supported dictionaries must retain the full-page copy set');
assert.deepEqual(dictionaryEntries[0].keys, dictionaryEntries[1].keys, 'Supported dictionaries must expose the same keys');
for (const locale of SUPPORTED_LOCALES) {
  const { t } = createTranslator(locale);
  for (const key of newLocaleKeys) assert.notEqual(t(key), key, `${locale} missing ${key}`);
}
for (const key of newLocaleKeys) assert.equal(i18nSource.split(`"${key}"`).length - 1, 2, `${key} must be explicit in both supported locales`);
assert.doesNotMatch(i18nSource, /const\s+ZH_TW\b/);
assert.doesNotMatch(i18nSource, /"(?:catalog\.category|settings\.resource\.category)":\s*"分類"/);
assert.doesNotMatch(i18nSource, /"resource\.netsuite\.description":\s*"[^"]*工作區/);
assert.equal(createTranslator('zh-TW').locale, 'zh-CN');
...
for (const [value, label] of [['zh-CN', '简体中文'], ['en-US', 'English']]) {
  assert.ok(index.includes(`<option value="${value}">${label}</option>`));
}
assert.ok(!index.includes('<option value="zh-TW">'));
assert.ok(!index.includes('繁體中文'));
...
setSearch('?role=admin&lang=zh-TW');
localStorage.setItem(LOCALE_STORAGE_KEY, 'en-US');
assert.equal(resolveLocale(), 'zh-CN');
const legacyUrl = new URL(localeUrl('https://prototype.invalid/?role=admin&surface=workspace&viewAs=finance', 'zh-Hant-TW'));
assert.equal(legacyUrl.searchParams.get('lang'), 'zh-CN');
assert.equal(legacyUrl.searchParams.get('role'), 'admin');
assert.equal(legacyUrl.searchParams.get('surface'), 'workspace');
assert.equal(legacyUrl.searchParams.get('viewAs'), 'finance');
localStorage.clear();
setSearch('?role=admin');
localStorage.setItem(LOCALE_STORAGE_KEY, 'zh-Hant-TW');
assert.equal(resolveLocale(), 'zh-CN');
localStorage.clear();
Object.defineProperty(globalThis, 'navigator', { value: { languages: ['en-US'], language: 'en-US' }, configurable: true });
assert.equal(resolveLocale(), 'en-US');
Object.defineProperty(globalThis, 'navigator', { value: { languages: ['zh-Hant-TW'], language: 'zh-Hant-TW' }, configurable: true });
assert.equal(resolveLocale(), 'zh-CN');
...
assert.ok(appSource.includes("locale==='zh-CN'?'zh-Hans-CN':'en-US'"));
assert.ok(!appSource.includes('zh-Hant-TW'));
...
assert.ok(localeResolver.indexOf('navigator.language') < localeResolver.lastIndexOf("'zh-CN'"));
