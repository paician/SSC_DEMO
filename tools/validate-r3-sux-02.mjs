...
// 15-16. Locale priority is preserved while supported output is limited to zh-CN and en-US.
...
assert.match(i18nSource, /SUPPORTED_LOCALES\s*=\s*Object\.freeze\(\['zh-CN', 'en-US'\]\)/);
assert.match(i18nSource, /DICTS\s*=\s*Object\.freeze\(\{ 'zh-CN': ZH_CN, 'en-US': EN \}\)/);
assert.doesNotMatch(i18nSource, /const\s+ZH_TW\b/);
for (const value of ['zh-CN', 'en-US']) assert.ok(index.includes(`<option value="${value}">`));
assert.ok(!index.includes('<option value="zh-TW">'));
assert.ok(!index.includes('繁體中文'));
assert.match(i18nSource, /startsWith\('zh-tw'\)[^\n]+return 'zh-CN'/);
